# -*- coding: utf-8 -*-
"""TOTAL PREDICTION.ipynb with Bi_GRU model"""
...
from tensorflow.keras.layers import Input, GRU, Dense, Flatten, Bidirectional
...
def create_sequence(dataset):
    return dataset.reshape(-1, 1, 1)  # GRU expects [samples, timesteps, features]
...
print("Step 6: Postprocessing, metrics, and plotting")
...
f_true = b * np.exp(np.array(fl)[idx] - 1)
f_pred = b * np.exp(Y_new[:, 0] - 1)

mae = mean_absolute_error(f_true, f_pred)
rmse = mean_squared_error(f_true, f_pred, squared=False)
r2 = r2_score(f_true, f_pred)

# Visualization
plt.scatter(after_burst['!Time'], after_burst['Flux'])
plt.plot(X_new, Y_new[:, 0], color='green', linestyle=(0, (0.1, 2)), dash_capstyle='round', linewidth=7)
plt.xscale('log')
plt.yscale('log')
plt.xlabel("t (s)")
plt.ylabel("Flux (erg/cm^2/s)")
plt.legend(['Original Points', 'Predicted Points'])
plt.title("Flux vs Time Prediction for GRB060206 (GRU Model)")
plt.show()
...
np.save("X_new_gru.npy", X_new)
np.save("Y_new_gru.npy", Y_new)
